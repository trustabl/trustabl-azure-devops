# Evaluating Trustabl on Azure Pipelines
...
- task: Trustabl@0
  inputs:
    severityThreshold: 'high'        # report-only: omit this
```

Inputs are **camelCase**: `severityThreshold`, `riskScoreThreshold`, `strict`,
`detectors`, `version`.
...
|---|---|
| **Pipeline log** | The readiness panel |
| **Build artifacts** | `trustabl.json` + `trustabl.sarif`, when `publishArtifact` is enabled |

Pair the SARIF output with a SARIF viewer extension if you want findings rendered
in the build summary rather than read out of the artifact.

For a trial, leave the thresholds unset and publish the artifact.
